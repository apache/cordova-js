 ...
 */
var webworks = require('phonegap/plugin/webworks/manager'),
    /**
     * Private list of HTML 5 audio objects, indexed by the PhoneGap media object ids
     */
    audioObjects = {},
    retInvalidAction = function () { 
        return { "status" : PhoneGap.callbackStatus.INVALID_ACTION, "message" : "Action not found" };
    },
    retAsyncCall = function () {
        return { "status" : PhoneGap.callbackStatus.NO_RESULT, "message" : "WebWorks Is On It" };
    },
    cameraAPI = {
    ...
    networkAPI = {
        execute: function (webWorksResult, action, args, win, fail) {
            ...
            var connectionType = require("phonegap/plugin/Connection").NONE,
                eventType = "offline",
                callbackID,
                request;
            ...
            if (blackberry.system.hasDataCoverage()) {
                connectionType = require("phonegap/plugin/Connection").WIFI;
                eventType = "online";
            }
