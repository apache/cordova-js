...
Battery.prototype._status = function(info) {
    ...
    var level = info.level;
		if (me._level !== level || me._isPlugged !== info.isPlugged) {
			...
			this._fire("batterystatus", info);

			// Fire low battery event
			if (level === 20 || level === 5) {
				if (level === 20) {
					this._fire("batterylow", info);
				}
				else {
					this._fire("batterycritical", info);
				}
			}
		}
		me._level = level;
...
Battery.prototype._fire = function(type, data) {
  ...
  if (e == 'batterystatus') {
    iterate(this._batteryListener, data);
  } else if (e == 'batterylow') {
    iterate(this._lowListener, data);
  } else if (e == 'batterycritical') {
    iterate(this._criticalListener, data);
  }
};
