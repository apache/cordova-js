...
describe('cordova', function () {
    describe('callbackFromNative', function () {
        it('Test#001 : should fire the cordovacallbackerror event if the callback throws', function () {
            var id = 'myID';
            var testError = new Error('errorMessage');
            cordova.callbacks[id] = {
                success: function () { throw testError; }
            };

            spyOn(cordova, 'fireWindowEvent');

            expect(function () {
                cordova.callbackFromNative(id, true, cordova.callbackStatus.OK, null, false);
            }).toThrow(testError);
